...
\begin{itemize}
    \item \textbf{CIC-UNSW-NB15} \cite{Mohammadian2024cicunsw}: This augmented dataset generates flows from the original 100GB UNSW-NB15 captures using CICFlowMeter. It incorporates nine attack categories (e.g., Fuzzers, Analysis, Backdoor, DoS, Exploits, Generic, Reconnaissance, Shellcode, Worms) alongside benign traffic. Malicious flows were retained while benign flows were sampled to preserve an 80:20 benign-to-malicious ratio, yielding 358,332 benign flows and 86,527 malicious flows for a realistic evaluation.
...
Table \ref{tab:results} summarizes the node-level detection performance across the CIC-UNSW-NB15 dataset. SIREN achieves a superior Macro-F1 score of 0.972 while suppressing the False Alarm Rate to an unprecedented 0.005 (0.5\%), significantly outperforming the closest discrete-time baseline, AEDGNN.

\begin{table}[htpb]
\caption{Detection Performance Comparison on CIC-UNSW-NB15 Dataset}
\label{tab:results}
\centering
\begin{tabular}{lcccc}
\toprule
